...
public class SinmungoDetailPanel extends JPanel {
	...
	private void init() {
        ...
        container.add(sinmungoTableSection());
        
        add(container);
        setVisible(true);
	}
	
	private JPanel sinmungoTableSection() {
		// 감싸고 있는 패널 설정
...
//        ));
        ...
        gbc.gridy = 0;
        panel.add(createStyledLabel("사례제목 (DB)", headerFont), gbc);
        
        gbc.gridy = 1;
        panel.add(makeContentBox("사례 들어갈 자리 (DB)", contentFont, 10), gbc);
        
        gbc.gridy = 2;
        panel.add(createStyledLabel("답변", headerFont), gbc);

        gbc.gridy = 3;
        panel.add(makeContentBox("답변 들어갈 자리 (DB)", contentFont, 10), gbc);
